...
export default function KnowledgePage() {
  ...
  return (
    <div className="space-y-8">
      ...
      <section>
        <h2 className="mb-3 text-lg font-semibold text-slate-900">スキル別タスク</h2>
        {!ready ? (
            ...
            {skillMap.map(([skill, relatedTasks]) => (
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs text-slate-400">{relatedTasks.length}件</span>
                </div>
                <ul className="mt-3 space-y-1">
                  {relatedTasks.map((t) => (
                    <li key={t.id} className="truncate text-sm text-slate-600">
                      ・{t.title}
                    </li>
                  ))}
                </ul>
      ...
      <section>
        <h2 className="mb-3 text-lg font-semibold text-slate-900">参考資料リンク集</h2>
          ...
          <ul className="divide-y divide-slate-100 overflow-hidden rounded-xl border border-slate-200 bg-white">
            {allLinks.map((link, i) => (
              <li key={i} className="flex items-center justify-between gap-3 p-4">
                ...
                <span className="shrink-0 text-xs text-slate-400">{link.taskTitle}</span>
              </li>
            ))}
          </ul>
        )}
