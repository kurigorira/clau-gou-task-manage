...
export function AiAssist({ task }: { task: Task }) {
  ...
  if (!isConfigured) {
    return (
      <div className="rounded-lg border border-brand-200 bg-brand-50 px-3 py-2 text-xs text-slate-600">
        🤖{" "}
  ...
  return (
    <div className="rounded-xl border border-brand-200 bg-brand-50 p-3">
      <p className="text-xs font-semibold text-brand-700">🤖 AIアシスト</p>
      <div className="mt-2 flex flex-wrap gap-2">
        ...
        <button
          ...
          disabled={busy !== null}
          className="rounded-md border border-brand-300 px-3 py-1.5 text-xs font-semibold text-brand-700 hover:bg-brand-100 disabled:opacity-50"
        >
